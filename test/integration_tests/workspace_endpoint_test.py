...
def test_invite_user(create_workspace_fixture):
    ...
    response = requests.post(url=base_url + "workspaces/invitation", json=request_body, headers={"Authorization": token})
    assert response.status_code == 200
    created_resources.append({repo.Collection.INVITATION: src.services.invitation_service.get_invitation(workspace_id=str(create_workspace_fixture._id), invitee_email_address=invitee_email_address)._id})
...
def test_invite_user_already_in_workspace(create_workspace_fixture, create_user):
